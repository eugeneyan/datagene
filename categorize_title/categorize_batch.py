"""
Batch categorize products given a csv. Will provide three category options.
...
from categorize_title.build_model import create_ngram
from categorize_title.categorize_utils import load_dict, get_score, get_top_n_score
...
def create_options(df, title, tfidf_dict):
    ...
    Returns a dataframe with top three category options.
...
def save_results(df, output_dir, country):
    ...
    Save recategorized result to csv.
...
if __name__ == '__main__':
    ...
    # Load dictionary
    tfidf_dict, regional_to_path_dict = load_dict('data/{}/model'.format(date),
                                                  'categorization_dict_{}'.format(country))
